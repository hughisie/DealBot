...
class ScrapulaService:
    ...
    def _parse_response(self, asin: str, data: dict) -> ScrapulaProductInfo:
        ...
        try:
            ...
            # Get first available image - try multiple field names
            image_url = (
                data.get("image_1") or
                data.get("image_2") or
                data.get("image_3") or
                ...
                data.get("main_image") or
                data.get("primary_image")
            )

            # Log image status for debugging
            if image_url:
                logger.info(f"✅ Scrapula found image for {asin}: {image_url[:60]}...")
            else:
                logger.warning(f"⚠️ Scrapula returned NO image for {asin}. Available fields: {list(data.keys())[:10]}")
